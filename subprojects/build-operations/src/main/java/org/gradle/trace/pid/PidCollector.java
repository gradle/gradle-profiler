...
public class PidCollector {
    public static void collect(GradleInternal gradle, File outFile) {
        // Always write the file, whether we register a build service or not, so we also have a PID
        // file for TAPI model requests.
        ...
        if (GradleVersion.current().compareTo(GradleVersion.version("6.1")) >= 0) {
            if (JavaVersion.current().isJava9Compatible()) {
                AbstractPidCollectorBuildService.registerBuildService(
                        PidCollectorBuildService.class, gradle, outFile);
            } else {
                AbstractPidCollectorBuildService.registerBuildService(
                        PidCollectorJava8BuildService.class, gradle, outFile);
            }
        }
    }
}
