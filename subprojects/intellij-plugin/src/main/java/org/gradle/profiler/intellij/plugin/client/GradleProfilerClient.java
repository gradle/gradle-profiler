...
import org.gradle.profiler.intellij.plugin.system.IdeDataImportListener;
...
public class GradleProfilerClient {
    ...
    public IdeRequest listenForSyncRequests(Project project, GradleSystemListener gradleSystemListener, IdeDataImportListener ideDataImportListener) {
        ...
        while (shouldHandleNextEvent(request)) {
            handleGradleProfilerRequest(request, project, gradleSystemListener, ideDataImportListener);
    ...
    private void handleGradleProfilerRequest(IdeRequest request, Project project, GradleSystemListener gradleSystemListener, IdeDataImportListener ideDataImportListener) {
        switch (request.getType()) {
            case SYNC:
                handleSyncRequest(request, project, gradleSystemListener, ideDataImportListener);
                break;
            case CLEANUP_CACHE:
    ...
    private void handleSyncRequest(IdeRequest request, Project project, GradleSystemListener gradleSystemListener, IdeDataImportListener ideDataImportListener) {
        ...
        if (isStartup) {
            ...
            GradleSyncHelper.waitOnPreviousImportToFinish(ideDataImportListener);
        ...
        GradleSyncResult result = isStartup
            ? GradleSyncHelper.getStartupSyncResult(project, gradleSystemListener, ideDataImportListener)
            : GradleSyncHelper.startManualSync(project, gradleSystemListener, ideDataImportListener);
