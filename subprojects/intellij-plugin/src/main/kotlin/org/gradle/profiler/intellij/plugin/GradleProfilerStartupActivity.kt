...
class GradleProfilerStartupActivity : ProjectActivity {
    ...
    override suspend fun execute(project: Project) {
        ...
        if (System.getProperty(PROFILER_PORT_PROPERTY) != null) {
            ...
            // Register Gradle task listener first so we catch any auto-triggered sync.
            ...
            // Subscribe the IDE data import listener via the project message bus so we catch
            // the IDE half of each sync (module/facet setup after the Gradle build completes).
            val ideDataImportListener = IdeDataImportListener()
            val messageBusConnection = project.messageBus.connect()
            messageBusConnection.subscribe(ProjectDataImportListener.TOPIC, ideDataImportListener)

            project.setTrusted(true)

            ApplicationManager.getApplication().executeOnPooledThread {
                try {
                    val lastRequest = listenForSyncRequests(project, gradleSystemListener, ideDataImportListener)
    ...
    private fun listenForSyncRequests(project: Project, gradleStartupListener: GradleSystemListener, ideDataImportListener: IdeDataImportListener): IdeRequest {
        val port = Integer.getInteger(PROFILER_PORT_PROPERTY)
        Client(port).use {
            return GradleProfilerClient(it).listenForSyncRequests(project, gradleStartupListener, ideDataImportListener)
        }
    }
}
