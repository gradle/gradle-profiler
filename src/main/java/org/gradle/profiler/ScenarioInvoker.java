...
public abstract class ScenarioInvoker<
        T extends ScenarioDefinition, R extends BuildInvocationResult> {
    /** Runs a scenario and collects the results. */
    public final void run(
            T scenario, InvocationSettings settings, BenchmarkResultCollector collector)
    ...
    /** Runs a scenario and collects the results. */
    abstract void doRun(T scenario, InvocationSettings settings, Consumer<R> resultConsumer)
            throws IOException, InterruptedException;
    ...
    protected R runMeasured(
            BuildContext buildContext,
            BuildMutator buildMutator,
            BuildStepAction<? extends R> action,
            Consumer<? super R> consumer) {
        R result =
                new RunBuildStepAction<R>(action, buildMutator).run(buildContext, BuildStep.BUILD);
