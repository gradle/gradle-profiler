...
import org.gradle.profiler.idea.IdeaSyncAction;
import org.gradle.profiler.idea.IdeaSyncScenarioDefinition;
...
class ScenarioLoader {
    ...
    // todo idea sync supports it, rename?
    ...
    private static final Map<String, BuildMutatorConfigurator> BUILD_MUTATOR_CONFIGURATORS = ImmutableMap.<String, BuildMutatorConfigurator>builder()
        ...
        .put(CLEAR_DIR, new ClearDirectoryMutator.Configurator())
    ...
    private static final List<String> ALL_SCENARIO_KEYS = ImmutableList.<String>builder()
        ...
        .addAll(IdeaSyncScenarioDefinition.getAllKeys())
    ...
    static List<ScenarioDefinition> loadScenarios(File scenarioFile, InvocationSettings settings, GradleBuildConfigurationReader inspector) {
        ...
        for (String scenarioName : selectedScenarios) {
            ...
            definitions.addAll(
                getScenarioDefinitions(scenarioFile, settings, scenarioName, scenario, title, buildCount, scenarioBaseDir, inspector)
            );
        }
    ...
    private static List<ScenarioDefinition> getScenarioDefinitions(
        File scenarioFile,
        InvocationSettings settings,
        String scenarioName, Config scenario, String title,
        int buildCount,
        File scenarioBaseDir,
        GradleBuildConfigurationReader inspector
    ) {
        if (scenario.hasPath(BAZEL) && settings.isBazel()) {
            return getBazelScenarioDefinitions(scenarioFile, settings, scenarioName, scenario, title, buildCount, scenarioBaseDir);
        } else if (scenario.hasPath(BUCK) && settings.isBuck()) {
            return getBuckScenarioDefinitions(scenarioFile, settings, scenarioName, scenario, title, buildCount, scenarioBaseDir);
        } else if (scenario.hasPath(MAVEN) && settings.isMaven()) {
            return getMavenScenarioDefinitions(scenarioFile, settings, scenarioName, scenario, title, buildCount, scenarioBaseDir);
        } else if (!settings.isBazel() && !settings.isBuck() && !settings.isMaven()) {
            return getGradleScenarioDefinitions(scenarioFile, settings, scenarioName, scenario, title, buildCount, scenarioBaseDir, inspector);
        } else {
            return Collections.emptyList();
        }
    }

    private static List<ScenarioDefinition> getGradleScenarioDefinitions(
        File scenarioFile,
        InvocationSettings settings,
        String scenarioName,
        Config scenario,
        ...
        for (GradleBuildConfiguration version : versions) {
            ...
            ScenarioDefinition scenarioDefinition = getScenarioDefinition(scenario, gradleScenarioDefinition);
            scenarioDefinitions.add(scenarioDefinition);
        }
        return scenarioDefinitions;
    }

    private static List<ScenarioDefinition> getBazelScenarioDefinitions(
        File scenarioFile,
        InvocationSettings settings,
        String scenarioName,
        Config scenario,
        ...
        Config executionInstructions = getExecutionInstructions(scenarioFile, settings, scenarioName, scenario, BAZEL, BAZEL_KEYS);
        ...
        List<BuildMutator> mutators = getMutators(scenario, scenarioName, settings, warmUpCount, buildCount);
        return ImmutableList.of(new BazelScenarioDefinition(scenarioName, title, targets, mutators, warmUpCount, buildCount, outputDir, bazelHome));
    }

    private static List<ScenarioDefinition> getBuckScenarioDefinitions(
        File scenarioFile,
        InvocationSettings settings,
        String scenarioName,
        Config scenario,
        ...
        Config executionInstructions = getExecutionInstructions(scenarioFile, settings, scenarioName, scenario, BUCK, BUCK_KEYS);
        List<String> targets = ConfigUtil.strings(executionInstructions, TARGETS);
        ...
        int warmUpCount = getWarmUpCount(settings, scenario);
        List<BuildMutator> mutators = getMutators(scenario, scenarioName, settings, warmUpCount, buildCount);
        return ImmutableList.of(new BuckScenarioDefinition(scenarioName, title, targets, type, mutators, warmUpCount, buildCount, outputDir, buckHome));
    }

    private static List<ScenarioDefinition> getMavenScenarioDefinitions(
        File scenarioFile,
        InvocationSettings settings,
        String scenarioName,
        Config scenario,
        ...
        Config executionInstructions = getExecutionInstructions(scenarioFile, settings, scenarioName, scenario, MAVEN, MAVEN_KEYS);
        List<String> targets = ConfigUtil.strings(executionInstructions, TARGETS);
    ...
    private static ScenarioDefinition getScenarioDefinition(Config scenario, GradleScenarioDefinition gradleScenarioDefinition) {
        return scenario.hasPath(ANDROID_STUDIO_SYNC) ? newStudioGradleScenarioDefinition(gradleScenarioDefinition, scenario)
            : scenario.hasPath(IdeaSyncScenarioDefinition.IDEA_SYNC_KEY) ? newIdeaGradleScenarioDefinition(gradleScenarioDefinition, scenario)
    ...
    private static IdeaGradleScenarioDefinition newIdeaGradleScenarioDefinition(GradleScenarioDefinition gradleScenarioDefinition, Config scenario) {
        Config ideaSyncConfig = scenario.getConfig(IdeaSyncScenarioDefinition.IDEA_SYNC_KEY);
        return new IdeaGradleScenarioDefinition(gradleScenarioDefinition, IdeaSyncScenarioDefinition.ofConfig(ideaSyncConfig));
    }
    ...
    public static GradleBuildInvoker invoker(Config config, GradleBuildInvoker defaultValue, BuildAction buildAction) {
        ...
        boolean sync = buildAction instanceof AndroidStudioSyncAction || buildAction instanceof IdeaSyncAction;
        if (sync) {
            invoker = getIdeSyncInvoker(config, buildAction instanceof AndroidStudioSyncAction);
        }
    ...
    private static GradleBuildInvoker getIdeSyncInvoker(Config config, boolean isAndroidStudio) {
        ...
        if (isAndroidStudio) {
            ...
            if (schedule == null || schedule == Schedule.SCENARIO) {
                return GradleBuildInvoker.IdeaCleanCacheBeforeScenario;
            } else if (schedule == Schedule.BUILD) {
                return GradleBuildInvoker.IdeaCleanCacheBeforeBuild;
            } else {
                throw new IllegalArgumentException(String.format("Unsupported cleanup schedule for '%s': '%s'", CLEAR_ANDROID_STUDIO_CACHE_BEFORE, schedule));
            }
        }
    ...
    private static BuildAction getBuildAction(Config scenario, String scenarioName, File scenarioFile, InvocationSettings invocationSettings) {
        ...
        boolean isSync = scenario.hasPath(ANDROID_STUDIO_SYNC) || scenario.hasPath(IdeaSyncScenarioDefinition.IDEA_SYNC_KEY);
        ...
        if (scenario.hasPath(IdeaSyncScenarioDefinition.IDEA_SYNC_KEY)) {
            if (invocationSettings.getIdeaSyncInvocationSettings() == null) {
                throw new IllegalArgumentException("IDEA version by --idea-version and home dir for installers by --idea-home should be specified when measuring IDEA sync.");
            }
            return new IdeaSyncAction();
        }
