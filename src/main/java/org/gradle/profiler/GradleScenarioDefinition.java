...
public class GradleScenarioDefinition extends ScenarioDefinition {
    ...
    public void visitProblems(InvocationSettings settings, Consumer<String> reporter) {
        if (getWarmUpCount() < 1) {
            reporter.accept(
                    "You can not skip warm-ups when profiling or benchmarking a Gradle build. Use --no-daemon or --cold-daemon if you want to profile or benchmark JVM startup");
        }
        if (settings.isMeasureGarbageCollection() && isBuildServiceUnsupported()) {
            reporter.accept(
                    "Measuring garbage collection is only supported for Gradle 6.1-milestone-3 and later");
        }
        if (settings.isMeasureConfigTime() && isBuildServiceUnsupported()) {
            reporter.accept(
                    "Measuring build configuration is only supported for Gradle 6.1-milestone-3 and later");
        }
    ...
    private boolean isBuildServiceUnsupported() {
        return buildConfiguration
                        .getGradleVersion()
                        .compareTo(GradleVersion.version("6.1-milestone-3"))
                < 0;
    }
    ...
    protected void printDetail(PrintStream out) {
        out.println(
                "  "
                        + getBuildConfiguration().getGradleVersion()
                        + " ("
                        + getBuildConfiguration().getGradleHome()
                        + ")");
        ...
        if (!measuredBuildOperations.isEmpty()) {
            out.println(
                    "  Measured build operations: "
                            + measuredBuildOperations.stream()
