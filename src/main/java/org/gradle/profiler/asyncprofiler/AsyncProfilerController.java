...
public class AsyncProfilerController
    ...
    public AsyncProfilerController(
            AsyncProfilerConfig profilerConfig, ScenarioSettings scenarioSettings) {
        this.profilerConfig = profilerConfig;
        this.scenarioSettings = scenarioSettings;
        ImmutableList.Builder<SanitizeFunction> sanitizers =
        ...
        this.flamegraphSanitizer =
                new FlameGraphSanitizer(sanitizers.build().toArray(new SanitizeFunction[0]));
    ...
    public void startRecording(String pid) throws IOException, InterruptedException {
        new CommandExec()
                .run(
                        ...
                        "start",
                        "-e",
                        profilerConfig.getEvent(),
                        "-i",
                        String.valueOf(profilerConfig.getInterval()),
                        "-j",
                        String.valueOf(profilerConfig.getStackDepth()),
                        "-b",
                        String.valueOf(profilerConfig.getFrameBuffer()),
                        pid);
    }
    ...
    public void stopRecording(String pid) throws IOException, InterruptedException {
        new CommandExec()
                .run(
                        ...
                        "stop",
                        "-o",
                        "collapsed=" + profilerConfig.getCounter().name().toLowerCase(Locale.ROOT),
                        "-a",
                        "-f",
                        stacks.getAbsolutePath(),
                        pid);
    }
    ...
    public void stopSession() {
        ...
        if (flamegraphGenerator.checkInstallation()) {
            File simplifiedStacks =
                    new File(
                            scenario.getOutputDir(),
                            scenario.getProfileName() + ".simplified-stacks.txt");
            ...
            File flamegraph =
                    new File(scenario.getOutputDir(), scenario.getProfileName() + "-flames.svg");
            flamegraphGenerator.generateFlameGraph(
                    ...
                    "1",
                    "--title",
                    titlePrefix + " Flame Graph",
                    "--countname",
                    unit);
            File iciclegraph =
                    new File(scenario.getOutputDir(), scenario.getProfileName() + "-icicles.svg");
            flamegraphGenerator.generateFlameGraph(
                    simplifiedStacks,
                    iciclegraph,
                    "--reverse",
