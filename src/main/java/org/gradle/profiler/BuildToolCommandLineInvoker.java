...
public abstract class BuildToolCommandLineInvoker<
                T extends BuildToolCommandLineScenarioDefinition, R extends BuildInvocationResult>
        ...
        try {
            for (int iteration = 1; iteration <= scenario.getWarmUpCount(); iteration++) {
                ...
                runMeasured(
                        buildContext,
                        mutator,
                        measureCommandLineExecution(
                                commandLine, settings.getProjectDir(), settings.getBuildLog()),
                        resultConsumer);
            }
    ...
    private BuildStepAction<R> measureCommandLineExecution(
            List<String> commandLine, File workingDir, File buildLog) {
