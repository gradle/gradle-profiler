...
class ProfilerIntegrationTest extends AbstractProfilerIntegrationTest {
    ...
    def "complains when using --single-shot with '#mutatorKey' mutation"() {
        given:
        def scenarioFile = file("benchmark.conf")
        scenarioFile.text = """
            assemble {
        ...
        if (targetFile != "build.gradle") {
            file(targetFile).text = "dummy"
        }

        when:
        new Main().
            run("--project-dir", projectDir.absolutePath, "--output-dir", outputDir.absolutePath, "--gradle-version", minimalSupportedGradleVersion, "--benchmark", "--single-shot", "--scenario-file", scenarioFile.absolutePath)

        then:
        thrown(IllegalArgumentException)

        and:
        output.contains("Source file mutations cannot be used with --single-shot")

        where:
        mutatorKey                               | targetFile
        "apply-build-script-change-to"           | "build.gradle"
        "apply-abi-change-to"                    | "src/main/java/Thing.java"
        "apply-non-abi-change-to"                | "src/main/java/Thing.java"
        ...
        "apply-android-layout-change-to"         | "src/main/res/layout/activity.xml"
        "apply-android-manifest-change-to"       | "src/main/AndroidManifest.xml"
        "apply-property-resource-change-to"      | "src/main/resources/app.properties"
        "apply-cpp-change-to"                    | "src/main/cpp/lib.cpp"
        "apply-h-change-to"                      | "src/main/cpp/lib.h"
        "apply-kotlin-composable-change-to"      | "src/main/kotlin/Thing.kt"
    }
