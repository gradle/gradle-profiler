...
If multiple versions are tested, then Gradle profiler determines whether there is an actual difference in the run times by using a [Mann-Whitney U-Test](https://de.wikipedia.org/wiki/Wilcoxon-Mann-Whitney-Test).
The result files contain the confidence of that the two samples actually have a different performance behavior - i.e. that one is faster or slower than the other.
